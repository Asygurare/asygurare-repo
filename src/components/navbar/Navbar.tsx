...
export default function Navbar() {
  ...
  useEffect(() => {
    supabaseClient.auth.getUser().then(({ data: { user } }) => setUser(user ?? null))
  ...
  return (
    <nav className="fixed top-0 w-full z-[100] px-7 pt-6 transition-all duration-500">
      <motion.div
        initial={{ y: -100 }}
        animate={{ y: 0 }}
        className={`max-w-7xl mx-auto rounded-2xl md:rounded-full transition-all duration-300 border ${isScrolled
              ...
              <>
                ...
                >
                  <motion.button
                    ...
                    whileTap={{ scale: 0.95 }}
                    className="bg-(--accents) text-white px-6 py-2.5 rounded-full text-sm font-bold shadow-lg shadow-[#4A7766]/20 flex items-center gap-2 group cursor-pointer"
                  >
                ...
                <button
                  type="button"
                  className="text-sm font-bold text-gray-600 hover:text-[#1a1a1a] transition-colors cursor-pointer"
                ...
                {user ? (
                  <>
                    ...
                    <button
                      ...
                      className="text-lg font-medium text-gray-500 text-left"
                    >
                      Cerrar sesión
                    </button>
                  </>
                  ...
                  <>
                    <Link href="/login" className="text-lg font-medium text-gray-500" onClick={() => setIsMobileMenuOpen(false)}>
                      Iniciar Sesión
                    </Link>
