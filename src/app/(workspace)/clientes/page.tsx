...
export default function ClientesPage() {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      let res;
      if (selectedCustomer) {
        res = await supabaseClient.from(DATABASE.TABLES.WS_CUSTOMERS).update(payload).eq('id', selectedCustomer.id)
      } else {
        res = await supabaseClient.from(DATABASE.TABLES.WS_CUSTOMERS).insert([payload])
      }
  ...
  const deleteCustomer = async (id: string, e: React.MouseEvent) => {
    ...
    const { error } = await supabaseClient.from(DATABASE.TABLES.WS_CUSTOMERS).delete().eq('id', id)
    if (!error) {
