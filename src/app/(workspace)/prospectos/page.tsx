...
export default function ProspectosFinalUltraPage() {
  ...
  const handleSaveLead = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const res = selectedLead 
      ? await supabaseClient.from(DATABASE.TABLES.WS_LEADS).update(payload).eq('id', selectedLead.id)
      : await supabaseClient.from(DATABASE.TABLES.WS_LEADS).insert([{ ...payload, stage: 'Primer contacto' }])
