...
export default function PagosPage() {
  ...
  const filteredPayments = useMemo(() => {
    return payments.filter(p => {
      const matchesSearch = p.policies?.customers?.full_name?.toLowerCase().includes(searchTerm.toLowerCase()) ||
                            p.policies?.policy_number?.toLowerCase().includes(searchTerm.toLowerCase())
                ...
                <tr key={p.id} className="hover:bg-gray-50 transition-all group">
                  <td className="p-10">
                    <div className="flex items-center gap-6">
                      ...
                      <div>
                        <p className="font-black text-black text-lg uppercase tracking-tighter leading-none">{p.policies?.customers?.full_name}</p>
                        <p className="text-[10px] font-bold text-black/40 uppercase mt-1 tracking-widest flex items-center gap-1">
                          <FileText size={12} /> {p.policies?.policy_number}
                        </p>
                      </div>
                    </div>
                  </td>
